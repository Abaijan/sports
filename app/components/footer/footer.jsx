...
export const Footer = () => {
    ...
    useEffect(() => {
        dynamicFetch('/footer').then((data) => setContacts(data));
    }, []);
        ...
        <footer className="blue">
            {
                contacts.map((contact) => (
                    <section key={contact.id} className="container flex justify-between py-10 items-center">
                        <section className="grid grid-rows-1  grid-cols-2 xl:grid-cols-6 xl:gap-16 gap-8 monrope">
                                    ...
                                    <section className="flex flex-col gap-2">
                            {
                                contact?.phone_numbers.map((number) => (
                                        <p key={number.id}>{number.phone}</p>
                            ...
                            <section className="flex gap-5">
                                {
                                    contact?.socials.map((social) => (
                                        <svg key={social.id} width="24" height="24" viewBox="0 0 24 24" fill="none"
