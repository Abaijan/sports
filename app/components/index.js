export { Footer } from "@/app/components/footer/footer";
export { Header } from "@/app/components/header/header";

//  Main

export { BannerHome } from "./Main/BannerHome";
export { HomeAbout } from '@/app/components/Main/homeAbout/HomeAbout';
export { Review } from '@/app/components/Main/review/Review';
export { New } from '@/app/components/Main/New';
export { PopularCategory } from '@/app/components/Main/PopularCategory';
export { ImageBanner } from '@/app/components/Main/ImageBanner';
export { SalesCards } from '@/app/components/Main/SalesCards';
export { Advantages } from '@/app/components/general/Advantages';
//General

export { BannerWithDescription} from "@/app/components/general/BannerWithDescription";


//card
export  { Card } from "@/app/components/Card/Card";

//whole sales
export { Discount } from "@/app/components/wholesalepurchase/Discount"
export { Explanation } from "@/app/components/AboutUs/explanation/Explanation"
export { ProductCards  } from "@/app/components/wholesalepurchase/ProducktCards"
export { HomeWholesale } from "@/app/components/wholesalepurchase/HomeWholesale"
export { WholeForm } from "@/app/components/wholesalepurchase/WholeForm"

//modals
export { ModalWindow } from "@/app/components/modalWindows/Module"
export { Basket } from "@/app/components/Basket/Basket"